...
class AdvantageEstimator(str, Enum):
    ...
    TREE_REINFORCE_PLUS_PLUS_BASELINE = "tree_reinforce_plus_plus_baseline"
    TREE_SEG_BASELINE = "tree_segment_reinforce_plus_plus_baseline"
    SEGPO = "spo"
    REMAX = "remax"
    RLOO = "rloo"
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.TREE_REINFORCE_PLUS_PLUS_BASELINE:
        advantages, returns = core_algos.compute_tree_reinforce_plus_plus_baseline_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            remove_no_std_subgroup=data.meta_info.get("subtree_std_ds", False),
            adv_weight_strategy=data.meta_info.get("adv_weight_strategy", "group_size"),
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.TREE_SEG_BASELINE:
        advantages, returns = core_algos.compute_tree_rfpp_progress_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            step_size=data.meta_info["step_max_token"],
            adv_eta=data.meta_info["segtree_adv_eta"],
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns  
    elif adv_estimator == AdvantageEstimator.SEGPO:
        advantages, returns = core_algos.compute_spo_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            step_size=data.meta_info["step_max_token"],
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.TREE_REINFORCE_PLUS_PLUS_BASELINE,
            AdvantageEstimator.TREE_SEG_BASELINE,
            AdvantageEstimator.SEGPO,
        ]:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.actor_rollout_ref.rollout.get("infer_mode", None) == "tree":
                        uid2original_idx = {}
                        for original_idx, uid in enumerate(batch.non_tensor_batch["uid"]):
                        ...
                        batch = batch[global_duplicated_idxs] # uuid 和 root node 一一对应
                    else:
                        batch = batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                ...
                if self.config.actor_rollout_ref.rollout.get("infer_mode", None) == "tree" and pad_size > 0:
