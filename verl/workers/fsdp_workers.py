            ...
            if self.config.rollout.name == "sglang_async":
                ...
                if hasattr(self.config.rollout, "infer_mode") and self.config.rollout.infer_mode == "tree" and not prompts.meta_info.get("validate", False):
                    ...
                    rollout_sampling_params = {
                        "fixed_step_width": self.config.rollout.get("tree_fixed_step_div", 2),
                        "random_first_div_min": self.config.rollout.get("tree_random_first_div_min", -1), # use this to control the first step divergence
                        "random_first_div_max": self.config.rollout.get("tree_random_first_div_max", -1), # use this to control the first step divergence
                        "max_depth": self.config.rollout.get("tree_max_depth", 6),
                        "min_width": self.config.rollout.get("tree_min_width", -1),
                        "max_width": self.config.rollout.get("tree_max_width", self.config.rollout.n),
                        "max_token_per_step": self.config.rollout.get("tree_max_token_per_step", 512),
                        "n": 1,
                        "detokenize": True, # 必须设定，用于检测是否生成答案，因为默认是 False
                        "calculate_entropy": self.config.rollout.get("tree_cal_entropy", False),
                        "logprobs": self.config.rollout.get("tree_n_logprob", 0),
                        "fallback_traj_policy": self.config.rollout.get("tree_fallback_traj_policy", "finished_first"),
                        "fallback_policy": self.config.rollout.get("tree_fallback_policy", "random"),
                        "fallback_window_size": self.config.rollout.get("tree_fallback_window_size", None),
                        "max_fallback_window": self.config.rollout.get("tree_max_fallback_window", -1),
                        "divergence_policy": self.config.rollout.get("tree_div_policy", "fixed_avg"),
                        "divergence_budget_control": self.config.rollout.get("tree_divergence_budget_control", "by_fixed_div"),
                        "logprob_div_temperature": self.config.rollout.get("tree_logprob_div_temperature", 1.0),
                        "cal_prob_block_size": self.config.rollout.get("cal_prob_block_size", 128),
                        "max_response_token": self.config.rollout.get("tree_max_response_token", -1),
                        "total_budget_policy": self.config.rollout.get("tree_total_budget_policy", "by_response_traj"),
                        "minimum_requests_per_gpu": self.config.rollout.get("tree_minimum_requests_per_gpu", -1),
                        "traj_filter_min_len": self.config.rollout.get("tree_traj_filter_min_len", -1),
                        "boxed_eos_policy": self.config.rollout.get("tree_boxed_eos_policy", "boxed_as_eos"),
                        "repetition_es_policy": self.config.rollout.get("tree_repetition_es_policy", "least_prioritized"),
                    }
                    prompts.meta_info.update({
                        "answer_start_tag_tokens": self.tokenizer.encode("\n\nIn conclusion, the final answer is", add_special_tokens=False)
                    })
                    if self.config.rollout.get("div_temp_scheduler", None) is not None:
                        rollout_sampling_params["logprob_div_temperature"] = prompts.meta_info["logprob_div_temperature"]
